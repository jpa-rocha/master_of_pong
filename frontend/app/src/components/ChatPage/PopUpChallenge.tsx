...
const ChallengeRoomPopup: React.FC<ChallengePopupProps> = ({ socket, userID, targetID, onClose }) => {
	...
	const handleModeSelectionChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
		const selectedValue = event.target.value;
		setMode(+selectedValue);
		console.log("+SelectedValue = ", +selectedValue);
		console.log("Mode.MasterOfPong = ", Mode.MasterOfPong);
		if (+selectedValue === Mode.MasterOfPong)
			setMasterToggle(true);
		else
			setMasterToggle(false);
	};

	const handleHyperSelectionChange = (event: React.MouseEvent<HTMLInputElement>) => {
		console.log("CHANGE HYPER");
		setHyper(!hyper);
	};
	
	const handleDodgeSelectionChange = (event: React.MouseEvent<HTMLInputElement>) => {
		console.log("CHANGE DODGE");
		setDodge(!dodge);
	};

	const handleCharacterSelectionChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
		const selectedValue = event.target.value;
		setCharacter(+selectedValue);
	};

	const handlePaddleSelectionChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
		const selectedValue = event.target.value;
		setPaddle(+selectedValue);
	};

	function handleSendChallenge() {
		socket.emit("sendChallenge", {userID: userID, targetID: targetID, mode: mode, hyper: hyper, dodge: dodge, character: character, paddle: paddle});
	}
  ...
  return (
    <>
		...
		<div className="PopBody">
			...
			<div>
			<select name="gamemode" id="gamemode" onChange={handleModeSelectionChange}>
			...
			<div>
			<input type="checkbox" id="hyper" onClick={handleHyperSelectionChange}></input>
			<label style={{marginRight: "10px"}} htmlFor="hyper"> Hyper Mode</label>
			<input type="checkbox" id="dodge" onClick={handleDodgeSelectionChange}></input>
			...
			{masterToggle ? (
				<>
					...
					<div>
					<select name="character" id="character" onChange={handleCharacterSelectionChange}>
					...
					<div>
						<select name="size" id="size" onChange={handlePaddleSelectionChange}>
