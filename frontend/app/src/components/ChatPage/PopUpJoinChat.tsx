...
const JoinChatRoomPopup: React.FC<JoinChatRoomPopupProps> = ({
  isOpen,
  onClose,
  onJoinChatRoom,
  socket,
  ...
  const handleJoinChatRoom = async (chatName: string, chatID: number) => {
    ...
    const foundPasswordInput = passwordInput.find(
      (item) => item.chatID === chatID
    );
    const passwordToSend = foundPasswordInput
  ...
  return (
    <>
      <div>
        <h3 className="PopHeader">Join Chat Room</h3>
            ...
            chatRooms.map((room) => (
              <div className="joinable-chat" key={room.id}>
                ...
                {room.channel === "private" ? (
                  <div>
                    <input
                      ...
                      onChange={(e) =>
                        setPasswordInputs(e.target.value, room.id)
                      }
                    />
                ...
                <button
                  ...
                  onClick={() => handleJoinChatRoom(room.title, room.id)}
                >
                  Join
                </button>
              </div>
